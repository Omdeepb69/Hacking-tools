...
def execute_remote_command(host, command, username=None, password=None, port=22):
    """Execute a shell command on a remote device using SSH"""
    if not username:
    ...
    try:
        ...
        print(f"Connected to {host}. Executing command: {command}")
...
def execute_remote_powershell(host, command, username=None, password=None):
    """Execute a PowerShell command on a remote Windows device using WinRM"""
    if not username:
        username = input(f"Enter username for {host}: ")
    ...
    try:
        ...
        # Execute using local PowerShell
        print(f"Connecting to {host} as {username}...")
        ...
        print(f"Error executing PowerShell command on {host}: {e}")
        return False

def main():
    parser = argparse.ArgumentParser(description='Network Device Tool')
    ...
    parser.add_argument('--exec', help='Execute a command on a remote device')
    parser.add_argument('--host', help='Target host for command execution')
    parser.add_argument('--user', help='Username for authentication')
    parser.add_argument('--password', help='Password for authentication')
    parser.add_argument('--port', type=int, default=22, help='Port for SSH connection')
    parser.add_argument('--windows', action='store_true', help='Use Windows PowerShell remoting instead of SSH')
    ...
    # Execute remote command if requested
    if args.exec and args.host:
        if args.windows:
            execute_remote_powershell(args.host, args.exec, args.user, args.password)
        else:
            execute_remote_command(args.host, args.exec, args.user, args.password, args.port)
    
    # If path and filename provided, write the file
    elif args.path and args.filename:
        ...
        if not args.discover and not (args.exec and args.host):
            print("\nNo operation specified. Available operations:")
            print("1. Discover network devices: --discover")
            print("2. Write a file: --path '//SERVER/share' --filename 'test.txt'")
            print("3. Execute command: --exec 'command' --host '192.168.1.100' --user 'username'")
            print("   For Windows targets: add --windows flag to use PowerShell remoting")
